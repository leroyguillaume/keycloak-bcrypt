...
public class BCryptPasswordHashProvider implements PasswordHashProvider {
    ...
    public String encode(final String rawPassword, final int iterations) {
        ...
        if (pw.length > BCrypt.Version.MAX_PW_LENGTH_BYTE) {
            return BCrypt.with(LongPasswordStrategies.truncate(BCrypt.Version.VERSION_2A)).hashToString(6, pw);
        } else {
    ...
    public boolean verify(final String rawPassword, final PasswordCredentialModel credential) {
        ...
        char[] pw = rawPassword.toCharArray();
        if (pw.length > BCrypt.Version.MAX_PW_LENGTH_BYTE) {
            final BCrypt.Result verifier = BCrypt.verifyer(BCrypt.Version.VERSION_2A, LongPasswordStrategies.truncate(BCrypt.Version.VERSION_2A)).verify(pw, hash);
